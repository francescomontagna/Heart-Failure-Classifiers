...
It is interesting to notice that $\hat{\beta}$ is an **unbiased estimator**, as we can easily proof
...
We start defining a complete additive model, using all available predictors, with R `glm()` function. Before doing that, we specify which predictors are categorical, or else, which are our *factors*.
...
```
We now want to focus on big model, to interpret the meaning of the estimated coefficient for a boolean predictor: extracting the estimated $\beta_{AN}$ for *anaemia*  we get
```{r}
...
In R this is done using `anova(smaller model, bigger model,test="Chisq")`.

We start comparing the small and big models
...
At this point, we can define a cross validation pipeline for our logistic regression models. Remember that what we see reported from K-fold CV is the arithmtetic average of the mean scores from each of the K subsets.  
...
C is a regularization parameter to trade off between error allowed and the margin minimization.
...
decrease the logodds along with the event probability. In this sense, older patiences are more at risk than younger ones, whereas those with smaller EF (*i.e.* which heart pumps less blood in the body) and renal disfunction (higher serum creatinine) are more in danger. As we can see, logistic regression model allows for good interpretability, which is postive in case we want our decision not to be driven by a black box.  

Looking at our example, where new sample is
```{r}
print(as.matrix(newdata))
```
we have a reasonable understanding that age and EF have prevailed in defining the prediction. Moreover, we can use some measures for uncertainty quantification: as we already shown when defining the complete model, we can derive confidence intervals for the coefficients with a certain level of statistical significance.  
